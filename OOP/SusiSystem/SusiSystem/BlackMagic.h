...
public:
	...
	/*size_t size() {
		size_t count = 0;
		I copyOfBegin = I(_begin);

		std::cout << " -> " << std::distance(_begin, _end) << '\n';
		while (copyOfBegin != _end) {
			++copyOfBegin;
			count++;
		}
...
container&
operator ~ (container vec) {
	static std::vector<typename container::iterator::value_type> vec2;
